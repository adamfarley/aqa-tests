	...
	if [ "${download_url}" != "" ]; then
		...
		do
			...
			do
				...
				echo "_ENCODE_FILE_NEW=UNTAGGED curl -iOLSk${curl_verbosity} ${curl_options} $file"
				...
				if [ -f "adoptopenjdk" ] && [ $uses_api_to_get_binary == "true" ]; then
				    file_size=$(stat --printf="%s" "adoptopenjdk")
			...
			if [ $download_exit_code != 0 ]; then
				...
				if [ -f "adoptopenjdk" ] && [ $uses_api_to_get_binary == "true" ]; then
				    ...
				    cat "adoptopenjdk"
				    echo "Note: We normally ignore these. This has not happened this time because the download failed even after retrying $count times."
				fi
